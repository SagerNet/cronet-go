...
// On 32-bit platforms, uint64 is passed as two 32-bit values (low, high) on the stack
func onReadCompletedCallback(self, request, info, buffer, bytesReadLow, bytesReadHigh uintptr) uintptr {
	handler := instanceOfURLRequestCallback(self)
	if handler == nil {
		return 0 // Post-destroy callback, silently ignore
	}
	bytesRead := uint64(bytesReadLow) | (uint64(bytesReadHigh) << 32)
