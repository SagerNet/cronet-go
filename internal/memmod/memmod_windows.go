...
func (module *Module) copySections(address, size uintptr, oldHeaders *IMAGE_NT_HEADERS) error {
	...
	for i := range sections {
		...
		if sections[i].PointerToRawData > uint32(size) ||
			sections[i].SizeOfRawData > uint32(size)-sections[i].PointerToRawData {
...
func (module *Module) executeTLS() {
	...
	if callback != 0 {
		const maxTLSCallbacks = 64 // Reasonable limit
...
func (module *Module) performBaseRelocation(delta uintptr) (relocated bool, err error) {
	...
	for relocationHdr.VirtualAddress > 0 {
		// Validate SizeOfBlock to prevent infinite loop
		if relocationHdr.SizeOfBlock < uint32(unsafe.Sizeof(*relocationHdr)) {
...
func LoadLibrary(data []byte) (module *Module, err error) {
	...
	// Alignment must be power of 2 and at least 1
	align := oldHeader.OptionalHeader.SectionAlignment
	if align == 0 || (align&(align-1)) != 0 {
		return nil, errors.New("Unaligned section")
	}
	lastSectionEnd := uintptr(0)
	sections := oldHeader.Sections()
	// Validate section count to prevent out-of-bounds access
	headerSize := uintptr(dosHeader.E_lfanew) + unsafe.Sizeof(IMAGE_NT_HEADERS{})
	maxSections := (size - headerSize) / unsafe.Sizeof(IMAGE_SECTION_HEADER{})
	if uintptr(oldHeader.FileHeader.NumberOfSections) > maxSections {
		return nil, errors.New("Too many sections for file size")
	}
...
func (module *Module) ProcAddressByName(name string) (uintptr, error) {
	...
	if idx, ok := module.nameExports[name]; ok {
		...
		funcRVA := *(*uint32)(a2p(module.codeBase + uintptr(exports.AddressOfFunctions) + uintptr(idx)*4))
		...
		if funcRVA >= directory.VirtualAddress && funcRVA < directory.VirtualAddress+directory.Size {
			return 0, errors.New("Forwarded exports not supported")
		}
		return module.codeBase + uintptr(funcRVA), nil
	}
